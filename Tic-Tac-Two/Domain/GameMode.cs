...
public record GameMode()
{
    ...
    {
        return Enum.GetNames(typeof(EGameMode)).Select(GetModeName).ToList();
    }
    
    public static List<EGameMode> GetGameModes()
    {
        return Enum.GetNames(typeof(EGameMode)).Select(GetMode).ToList();
    }
    ...
    private static EGameMode GetMode(string modeName)
    {
